...
import { modelingReserveParkovayaElectricData } from "../data/modelingReserveParkovayaElectric";
...
export function ModelingReserveParkovayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreserveparkovaya_electric', width, height);
  ...
  const allReserves = modelingReserveParkovayaElectricData.map(d => d.reserve);
  ...
  const allZero = modelingReserveParkovayaElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...modelingReserveParkovayaElectricData.map((d) => {
