...
import { modelingReserveLefortovoElectricData } from "../data/modelingReserveLefortovoElectric";
...
export function ModelingReserveLefortovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreservelefortovo_electric', width, height);
  ...
  const allReserves = modelingReserveLefortovoElectricData.map(d => d.reserve);
  ...
  const allZero = modelingReserveLefortovoElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...modelingReserveLefortovoElectricData.map((d) => {
