...
import { reserveVykhinoElectricData } from "../data/reserveVykhinoElectric";
...
export function ReserveVykhinoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/reservevykhino_electric', width, height);
  ...
  const allReserves = reserveVykhinoElectricData.map(d => d.reserve);
  ...
  const allZero = reserveVykhinoElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...reserveVykhinoElectricData.map((d) => {
