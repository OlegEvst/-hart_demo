...
import { balanceKrasnoselskayaElectricData } from "../data/balanceKrasnoselskayaElectric";
...
export function BalanceKrasnoselskayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balancekrasnoselskaya_electric', width, height);
  ...
  const allValues = [...balanceKrasnoselskayaElectricData.map(d => d.total_net), ...balanceKrasnoselskayaElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceKrasnoselskayaElectricData.map((d) => {
