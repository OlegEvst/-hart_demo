...
import { modelingReserveVykhinoElectricData } from "../data/modelingReserveVykhinoElectric";
...
export function ModelingReserveVykhinoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreservevykhino_electric', width, height);
  ...
  const allReserves = modelingReserveVykhinoElectricData.map(d => d.reserve);
  ...
  const allZero = modelingReserveVykhinoElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...modelingReserveVykhinoElectricData.map((d) => {
