...
import { reserve22Data } from "../data/reserve22";
...
export function Reserve22Chart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/reserve22', width, height);
  ...
  const annotationStemLengthNegative = width > 500 ? -22.5 : -15; // Увеличиваем на 50% при ширине > 500px
  const annotationStemLengthPositive = width > 500 ? 18 : 12; // Увеличиваем на 50% при ширине > 500px
  ...
  const commonOptions = {
    ...
    vAxis: {
      ...
      format: "decimal",
      ticks: [{ v: 0, f: "0" }], // Показываем только значение 0
    },
    titlePosition: "none",
  ...
  const data1 = [
    ...
    ...reserve22Data.map((d) => [
      d.year,
      d.reserve,
      (d.year === "2026" || d.year === "2030") ? "" : Math.round(d.reserve).toString(), // Убираем стандартные аннотации для 2026 и 2030
      d.year === "2026" ? 361.7 : null, // Отдельная серия для 361.7
      d.year === "2026" ? "361.7" : "", // Аннотация ниже
      d.year === "2030" ? -277.8 : null, // Отдельная серия для -277.8
      d.year === "2030" ? "-277.8" : "", // Аннотация выше
    ]),
  ];
  ...
  const options1 = {
    ...
    series: {
      ...
      1: { 
        ...
        pointSize: 0,
        annotations: {
          stem: { color: "transparent", length: annotationStemLengthNegative }, // Ниже точки для 361.7
        }
      },
      2: { 
        color: "#8B4513", 
        ...
        pointSize: 0,
        annotations: {
          stem: { color: "transparent", length: annotationStemLengthPositive }, // Выше точки для -277.8
        }
      },
    },
    vAxis: {
      ...commonOptions.vAxis,
      viewWindow: { min: -300, max: 400 },
      gridlines: { 
        color: "#F2F2F7", 
        count: 4,
      },
      format: "decimal",
      ticks: [0] as any, // Показываем только значение 0
    },
  };
