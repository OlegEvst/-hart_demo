...
import { balanceGolyanovoElectricData } from "../data/balanceGolyanovoElectric";
...
export function BalanceGolyanovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balancegolyanovo_electric', width, height);
  ...
  const allValues = [...balanceGolyanovoElectricData.map(d => d.total_net), ...balanceGolyanovoElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceGolyanovoElectricData.map((d) => {
