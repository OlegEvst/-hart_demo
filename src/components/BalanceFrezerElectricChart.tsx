...
import { balanceFrezerElectricData } from "../data/balanceFrezerElectric";
...
export function BalanceFrezerElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balancefrezer_electric', width, height);
  ...
  const allValues = [...balanceFrezerElectricData.map(d => d.total_net), ...balanceFrezerElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceFrezerElectricData.map((d) => {
