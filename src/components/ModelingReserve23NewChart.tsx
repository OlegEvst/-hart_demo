...
import { modelingReserve23NewData } from "../data/modelingReserve23New";
...
export function ModelingReserve23NewChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreserve23_new', width, height);
  ...
  const allReserves = modelingReserve23NewData.map(d => d.reserve);
  ...
  useEffect(() => {
    const addCustomAnnotation = () => {
      ...
      const totalYears = modelingReserve23NewData.length;
      ...
      const value2026 = modelingReserve23NewData.find(d => d.year === "2026");
      const yearIndex2026 = value2026 ? modelingReserve23NewData.findIndex(d => d.year === "2026") : -1;
  ...
  const data1 = [
    ...
    ...modelingReserve23NewData.map((d) => [
      d.year,
      d.reserve,
