...
import { modelingBalanceIzmaylovoElectricData } from "../data/modelingBalanceIzmaylovoElectric";
...
export function ModelingBalanceIzmaylovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingbalanceizmaylovo_electric', width, height);
  ...
  const allValues = [...modelingBalanceIzmaylovoElectricData.map(d => d.total_net), ...modelingBalanceIzmaylovoElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...modelingBalanceIzmaylovoElectricData.map((d) => {
