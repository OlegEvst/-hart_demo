...
import { balanceParkovayaElectricData } from "../data/balanceParkovayaElectric";
...
export function BalanceParkovayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balanceparkovaya_electric', width, height);
  ...
  const allValues = [...balanceParkovayaElectricData.map(d => d.total_net), ...balanceParkovayaElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceParkovayaElectricData.map((d) => {
