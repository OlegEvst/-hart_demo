...
import { reserveKrasnoselskayaElectricData } from "../data/reserveKrasnoselskayaElectric";
...
export function ReserveKrasnoselskayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/reservekrasnoselskaya_electric', width, height);
  ...
  const allReserves = reserveKrasnoselskayaElectricData.map(d => d.reserve);
  ...
  const allZero = reserveKrasnoselskayaElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...reserveKrasnoselskayaElectricData.map((d) => {
