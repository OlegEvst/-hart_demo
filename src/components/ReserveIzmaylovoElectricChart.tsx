...
import { reserveIzmaylovoElectricData } from "../data/reserveIzmaylovoElectric";
...
export function ReserveIzmaylovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/reserveizmaylovo_electric', width, height);
  ...
  // Используем сохраненные настройки, если они есть, иначе используем значения по умолчанию
  ...
  const allReserves = reserveIzmaylovoElectricData.map(d => d.reserve);
  ...
  const allZero = reserveIzmaylovoElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...reserveIzmaylovoElectricData.map((d) => {
