...
import { modelingBalanceBaskakovoElectricData } from "../data/modelingBalanceBaskakovoElectric";
...
export function ModelingBalanceBaskakovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingbalancebaskakovo_electric', width, height);
  ...
  const allValues = [...modelingBalanceBaskakovoElectricData.map(d => d.total_net), ...modelingBalanceBaskakovoElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...modelingBalanceBaskakovoElectricData.map((d) => {
