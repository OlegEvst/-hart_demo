...
import { modelingReserveIzmaylovoElectricData } from "../data/modelingReserveIzmaylovoElectric";
...
export function ModelingReserveIzmaylovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreserveizmaylovo_electric', width, height);
  ...
  const allReserves = modelingReserveIzmaylovoElectricData.map(d => d.reserve);
  ...
  const allZero = modelingReserveIzmaylovoElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...modelingReserveIzmaylovoElectricData.map((d) => {
