...
import { balanceTsimlyanskayaElectricData } from "../data/balanceTsimlyanskayaElectric";
...
export function BalanceTsimlyanskayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balancetsimlyanskaya_electric', width, height);
  ...
  const allValues = [...balanceTsimlyanskayaElectricData.map(d => d.total_net), ...balanceTsimlyanskayaElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceTsimlyanskayaElectricData.map((d) => {
