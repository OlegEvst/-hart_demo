...
import { modelingReserveProzhektorElectricData } from "../data/modelingReserveProzhektorElectric";
...
export function ModelingReserveProzhektorElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreserveprozhektor_electric', width, height);
  ...
  const allReserves = modelingReserveProzhektorElectricData.map(d => d.reserve);
  ...
  const allZero = modelingReserveProzhektorElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...modelingReserveProzhektorElectricData.map((d) => {
