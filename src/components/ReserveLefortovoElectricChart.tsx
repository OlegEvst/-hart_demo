...
import { reserveLefortovoElectricData } from "../data/reserveLefortovoElectric";
...
export function ReserveLefortovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/reservelefortovo_electric', width, height);
  ...
  const allReserves = reserveLefortovoElectricData.map(d => d.reserve);
  ...
  const allZero = reserveLefortovoElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...reserveLefortovoElectricData.map((d) => {
