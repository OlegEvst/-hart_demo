...
import { balanceVostochnayaElectricData } from "../data/balanceVostochnayaElectric";
...
export function BalanceVostochnayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balancevostochnaya_electric', width, height);
  ...
  const allValues = [...balanceVostochnayaElectricData.map(d => d.total_net), ...balanceVostochnayaElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceVostochnayaElectricData.map((d) => {
