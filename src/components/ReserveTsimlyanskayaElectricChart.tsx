...
import { reserveTsimlyanskayaElectricData } from "../data/reserveTsimlyanskayaElectric";
...
export function ReserveTsimlyanskayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/reservetsimlyanskaya_electric', width, height);
  ...
  const allReserves = reserveTsimlyanskayaElectricData.map(d => d.reserve);
  ...
  const allZero = reserveTsimlyanskayaElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...reserveTsimlyanskayaElectricData.map((d) => {
