...
import { modelingReservePerovoNewData } from "../data/modelingReservePerovoNew";
...
export function ModelingReservePerovoNewChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreserveperovo_new', width, height);
  ...
  // Уменьшаем max на 30% чтобы увеличить физическое расстояние между делениями от 0 до максимального значения
  const allReserves = modelingReservePerovoNewData.map(d => d.reserve);
  const maxReserve = Math.max(...allReserves);
  const baseMax = Math.ceil(maxReserve / 100) * 100 + 100;
  const vAxisMax = baseMax * 0.77; // Уменьшаем на ~23% чтобы увеличить расстояние между делениями на 30%
  ...
  const data1 = [
    ...
    ...modelingReservePerovoNewData.map((d) => [
      d.year,
      d.reserve,
      d.reserve.toFixed(1), // Показываем одно значение после запятой
    ]),
  ];
