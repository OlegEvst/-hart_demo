...
import { modelingBalanceLefortovoElectricData } from "../data/modelingBalanceLefortovoElectric";
...
export function ModelingBalanceLefortovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingbalancelefortovo_electric', width, height);
  ...
  const allValues = [...modelingBalanceLefortovoElectricData.map(d => d.total_net), ...modelingBalanceLefortovoElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...modelingBalanceLefortovoElectricData.map((d) => {
