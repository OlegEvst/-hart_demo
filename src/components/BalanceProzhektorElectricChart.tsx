...
import { balanceProzhektorElectricData } from "../data/balanceProzhektorElectric";
...
export function BalanceProzhektorElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balanceprozhektor_electric', width, height);
  ...
  const allValues = [...balanceProzhektorElectricData.map(d => d.total_net), ...balanceProzhektorElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceProzhektorElectricData.map((d) => {
