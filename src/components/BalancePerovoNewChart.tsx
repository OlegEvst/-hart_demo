...
import { balancePerovoNewData } from "../data/balancePerovoNew";
...
export function BalancePerovoNewChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balanceperovo_new', width, height);
  ...
  const chartAreaTop = savedConfig?.chartAreaTop ?? (isFixedSize ? "-12%" : (isWidth340 ? "-10%" : 0)); // Поднимаем график выше для 272x150 и 340x190
  ...
  // Вычисляем max для vAxis (min всегда 0 для заливки от нуля)
  const allValues = [...balancePerovoNewData.map(d => d.total_net), ...balancePerovoNewData.map(d => d.load)];
  const maxValue = Math.max(...allValues);
  const vAxisMax = Math.ceil(maxValue / 100) * 100 + 100;
  ...
  const options1 = {
    ...
    vAxis: {
      ...
      viewWindow: { 
        min: 0, // Заливка всегда начинается от нуля
        max: vAxisMax 
      },
      format: "decimal",
    },
