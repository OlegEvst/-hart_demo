...
import { modelingPerovoNewData } from "../data/modelingPerovoNew";
...
export function ModelingPerovoNewChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingperovo_new', width, height);
  ...
  const allValues = [...modelingPerovoNewData.map(d => d.total_net), ...modelingPerovoNewData.map(d => d.load)];
  ...
  const options1 = {
    ...
    vAxis: {
      ...commonOptions.vAxis,
      viewWindow: { min: 0, max: vAxisMax }, // Заливка всегда начинается от нуля
      gridlines: { color: "#F2F2F7", count: savedConfig?.vAxisGridlinesCount ?? 1 },
    },
  };
