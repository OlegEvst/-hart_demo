...
import { modelingFrezData } from "../data/modelingFrez";
...
export function ModelingFrezChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingfrez', width, height);
  ...
  // Опускаем график немного ниже для всех размеров экрана, чтобы точки не обрезались
  const chartAreaTop = savedConfig?.chartAreaTop ?? ("5%");
  const chartAreaBottom = savedConfig?.chartAreaBottom ?? (isSuperTiny ? 30 : (isTiny ? 25 : 20));
  // Компенсируем увеличение chartAreaTop уменьшением высоты
  const chartAreaHeight = savedConfig?.chartAreaHeight ?? (isSuperTiny ? "80%" : (isTiny ? "85%" : (isMobile ? "90%" : "93%")));
