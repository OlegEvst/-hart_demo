...
import { modelingReserveVostochnayaElectricData } from "../data/modelingReserveVostochnayaElectric";
...
export function ModelingReserveVostochnayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreservevostochnaya_electric', width, height);
  ...
  const allReserves = modelingReserveVostochnayaElectricData.map(d => d.reserve);
  ...
  const allZero = modelingReserveVostochnayaElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...modelingReserveVostochnayaElectricData.map((d) => {
