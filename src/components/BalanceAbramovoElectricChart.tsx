...
import { balanceAbramovoElectricData } from "../data/balanceAbramovoElectric";
...
export function BalanceAbramovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balanceabramovo_electric', width, height);
  ...
  const allValues = [...balanceAbramovoElectricData.map(d => d.total_net), ...balanceAbramovoElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceAbramovoElectricData.map((d) => {
