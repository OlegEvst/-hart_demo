...
import { reserveVostochnayaElectricData } from "../data/reserveVostochnayaElectric";
...
export function ReserveVostochnayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/reservevostochnaya_electric', width, height);
  ...
  const allReserves = reserveVostochnayaElectricData.map(d => d.reserve);
  ...
  const allZero = reserveVostochnayaElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...reserveVostochnayaElectricData.map((d) => {
