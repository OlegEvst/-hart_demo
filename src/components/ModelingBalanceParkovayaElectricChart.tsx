...
import { modelingBalanceParkovayaElectricData } from "../data/modelingBalanceParkovayaElectric";
...
export function ModelingBalanceParkovayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingbalanceparkovaya_electric', width, height);
  ...
  const allValues = [...modelingBalanceParkovayaElectricData.map(d => d.total_net), ...modelingBalanceParkovayaElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...modelingBalanceParkovayaElectricData.map((d) => {
