...
import { balanceBaskakovoElectricData } from "../data/balanceBaskakovoElectric";
...
export function BalanceBaskakovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balancebaskakovo_electric', width, height);
  ...
  const allValues = [...balanceBaskakovoElectricData.map(d => d.total_net), ...balanceBaskakovoElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceBaskakovoElectricData.map((d) => {
