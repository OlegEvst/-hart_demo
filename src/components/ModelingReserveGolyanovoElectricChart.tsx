...
import { modelingReserveGolyanovoElectricData } from "../data/modelingReserveGolyanovoElectric";
...
export function ModelingReserveGolyanovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreservegolyanovo_electric', width, height);
  ...
  const allReserves = modelingReserveGolyanovoElectricData.map(d => d.reserve);
  ...
  const allZero = modelingReserveGolyanovoElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...modelingReserveGolyanovoElectricData.map((d) => {
