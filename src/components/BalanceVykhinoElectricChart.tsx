...
import { balanceVykhinoElectricData } from "../data/balanceVykhinoElectric";
...
export function BalanceVykhinoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balancevykhino_electric', width, height);
  ...
  const allValues = [...balanceVykhinoElectricData.map(d => d.total_net), ...balanceVykhinoElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceVykhinoElectricData.map((d) => {
