...
import { modelingReserveBaskakovoElectricData } from "../data/modelingReserveBaskakovoElectric";
...
export function ModelingReserveBaskakovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreservebaskakovo_electric', width, height);
  ...
  const allReserves = modelingReserveBaskakovoElectricData.map(d => d.reserve);
  ...
  const allZero = modelingReserveBaskakovoElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...modelingReserveBaskakovoElectricData.map((d) => {
