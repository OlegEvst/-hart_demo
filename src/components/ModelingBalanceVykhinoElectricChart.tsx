...
import { modelingBalanceVykhinoElectricData } from "../data/modelingBalanceVykhinoElectric";
...
export function ModelingBalanceVykhinoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingbalancevykhino_electric', width, height);
  ...
  const allValues = [...modelingBalanceVykhinoElectricData.map(d => d.total_net), ...modelingBalanceVykhinoElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...modelingBalanceVykhinoElectricData.map((d) => {
