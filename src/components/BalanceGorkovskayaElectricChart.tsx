...
import { balanceGorkovskayaElectricData } from "../data/balanceGorkovskayaElectric";
...
export function BalanceGorkovskayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balancegorkovskaya_electric', width, height);
  ...
  const allValues = [...balanceGorkovskayaElectricData.map(d => d.total_net), ...balanceGorkovskayaElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceGorkovskayaElectricData.map((d) => {
