...
import { modelingBalanceVostochnayaElectricData } from "../data/modelingBalanceVostochnayaElectric";
...
export function ModelingBalanceVostochnayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingbalancevostochnaya_electric', width, height);
  ...
  const allValues = [...modelingBalanceVostochnayaElectricData.map(d => d.total_net), ...modelingBalanceVostochnayaElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...modelingBalanceVostochnayaElectricData.map((d) => {
