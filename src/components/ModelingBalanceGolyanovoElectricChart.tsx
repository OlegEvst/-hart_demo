...
import { modelingBalanceGolyanovoElectricData } from "../data/modelingBalanceGolyanovoElectric";
...
export function ModelingBalanceGolyanovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingbalancegolyanovo_electric', width, height);
  ...
  const allValues = [...modelingBalanceGolyanovoElectricData.map(d => d.total_net), ...modelingBalanceGolyanovoElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...modelingBalanceGolyanovoElectricData.map((d) => {
