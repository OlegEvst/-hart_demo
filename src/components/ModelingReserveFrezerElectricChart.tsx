...
import { modelingReserveFrezerElectricData } from "../data/modelingReserveFrezerElectric";
...
export function ModelingReserveFrezerElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreservefrezer_electric', width, height);
  ...
  const allReserves = modelingReserveFrezerElectricData.map(d => d.reserve);
  ...
  const allZero = modelingReserveFrezerElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...modelingReserveFrezerElectricData.map((d) => {
