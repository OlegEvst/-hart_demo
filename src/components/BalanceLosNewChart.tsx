...
import { balanceLosNewData } from "../data/balanceLosNew";
...
export function BalanceLosNewChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balancelos_new', width, height);
  
  const isTiny = width <= 330;
  const isMobile = width < 600;
  const isFixedSize = width >= 270 && width <= 280; // Для экрана 272x150
  const isWidth340 = width >= 340 && width <= 350; // Для экрана 340x190
  ...
  const chartAreaLeft = getDefaultValue('chartAreaLeft', { '276x155': '8%', '344x193': '5%', '900x250': '3%' });
  const chartAreaRight = getDefaultValue('chartAreaRight', { '276x155': '5%', '344x193': '5%', '900x250': '3%' });
  const chartAreaTop = getDefaultValue('chartAreaTop', { '276x155': '-30%', '344x193': '-10%', '900x250': '-25%' });
  const chartAreaBottom = getDefaultValue('chartAreaBottom', { '276x155': 35, '344x193': 35, '900x250': 20 });
  const chartAreaHeight = getDefaultValue('chartAreaHeight', { '276x155': '98%', '344x193': '98%', '900x250': '98%' });
  const chartAreaWidth = getDefaultValue('chartAreaWidth', { '276x155': '90%', '344x193': '94%', '900x250': '94%' });

  const legendLeftPadding = getDefaultValue('legendLeftPadding', { '276x155': '8%', '344x193': '5%', '900x250': '3%' });
  const legendMarginTop = getDefaultValue('legendMarginTop', { '276x155': '-7px', '344x193': '0px', '900x250': '8px' });
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
        ...
        <span style={{ 
          ...
          Нагрузка существующих объектов, тыс.куб.м/сутки
        </span>
      </div>
    </div>
  );
  ...
  const commonOptions = {
    ...
    hAxis: {
      ...
      // Убираем отступы для маленьких разрешений
    ...
    vAxis: {
      ...
      gridlines: { color: "#F2F2F7", count: getDefaultValue('vAxisGridlinesCount', { '276x155': 3, '344x193': 3, '900x250': 3 }) },
      baselineColor: "#E5E5EA",
      // Убираем отступы для маленьких разрешений
      ...(isFixedSize || isWidth340 ? { 
        textPosition: "none" as const,
  ...
  // Используем сохраненные значения или значения по умолчанию из вашей конфигурации
  const vAxisMin = getDefaultValue('vAxisMin', { '276x155': 1400, '344x193': 1470, '900x250': 1480 });
  const vAxisMax = getDefaultValue('vAxisMax', { '276x155': 2120, '344x193': 2120, '900x250': 2100 });
  ...
  const options1 = {
    ...
    series: {
      ...
      1: { 
        ...
        areaOpacity: 0.15,
        annotations: {
          stem: { color: "transparent", length: -annotationStemLength * 2.65 }, // Отрицательное значение размещает под точками, увеличенный отступ
        }
      },
    },
    vAxis: {
      ...
      gridlines: { color: "#F2F2F7", count: getDefaultValue('vAxisGridlinesCount', { '276x155': 1, '344x193': 1, '900x250': 1 }) },
      baselineColor: "#E5E5EA",
      // Убираем отступы для маленьких разрешений
      ...(isFixedSize || isWidth340 ? { 
        textPosition: "none" as const,
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div style={{
          ...
          borderRadius: "0",
          padding: `${getDefaultValue('containerPaddingTop', { '276x155': '1.4%', '344x193': '2px', '900x250': '0.4%' })} 0 7px 0`, // padding-bottom: 5px базовый + 2px margin = 7px
          ...
          <div style={{ 
            width: "100%", 
            height: getDefaultValue('chartContainerHeight', { '276x155': '125px', '344x193': '150px', '900x250': '188px' }),
          }}>
            <Chart
              chartType="AreaChart"
              width="100%"
