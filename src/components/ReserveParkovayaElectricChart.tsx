...
import { reserveParkovayaElectricData } from "../data/reserveParkovayaElectric";
...
export function ReserveParkovayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/reserveparkovaya_electric', width, height);
  ...
  const allReserves = reserveParkovayaElectricData.map(d => d.reserve);
  ...
  const allZero = reserveParkovayaElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...reserveParkovayaElectricData.map((d) => {
