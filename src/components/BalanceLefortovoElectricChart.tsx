...
import { balanceLefortovoElectricData } from "../data/balanceLefortovoElectric";
...
export function BalanceLefortovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balancelefortovo_electric', width, height);
  ...
  const allValues = [...balanceLefortovoElectricData.map(d => d.total_net), ...balanceLefortovoElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceLefortovoElectricData.map((d) => {
