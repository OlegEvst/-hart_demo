...
import { modelingBalanceAbramovoElectricData } from "../data/modelingBalanceAbramovoElectric";
...
export function ModelingBalanceAbramovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingbalanceabramovo_electric', width, height);
  ...
  const allValues = [...modelingBalanceAbramovoElectricData.map(d => d.total_net), ...modelingBalanceAbramovoElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...modelingBalanceAbramovoElectricData.map((d) => {
