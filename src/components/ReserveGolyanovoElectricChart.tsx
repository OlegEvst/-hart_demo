...
import { reserveGolyanovoElectricData } from "../data/reserveGolyanovoElectric";
...
export function ReserveGolyanovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/reservegolyanovo_electric', width, height);
  ...
  const allReserves = reserveGolyanovoElectricData.map(d => d.reserve);
  ...
  const allZero = reserveGolyanovoElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...reserveGolyanovoElectricData.map((d) => {
