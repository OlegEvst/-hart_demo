...
import { reserveAbramovoElectricData } from "../data/reserveAbramovoElectric";
...
export function ReserveAbramovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/reserveabramovo_electric', width, height);
  ...
  const allReserves = reserveAbramovoElectricData.map(d => d.reserve);
  ...
  const allZero = reserveAbramovoElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...reserveAbramovoElectricData.map((d) => {
