...
import { modelingReserveAbramovoElectricData } from "../data/modelingReserveAbramovoElectric";
...
export function ModelingReserveAbramovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreserveabramovo_electric', width, height);
  ...
  const allReserves = modelingReserveAbramovoElectricData.map(d => d.reserve);
  ...
  const allZero = modelingReserveAbramovoElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...modelingReserveAbramovoElectricData.map((d) => {
