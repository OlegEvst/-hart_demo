...
import { modelingBalanceGorkovskayaElectricData } from "../data/modelingBalanceGorkovskayaElectric";
...
export function ModelingBalanceGorkovskayaElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingbalancegorkovskaya_electric', width, height);
  ...
  const allValues = [...modelingBalanceGorkovskayaElectricData.map(d => d.total_net), ...modelingBalanceGorkovskayaElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...modelingBalanceGorkovskayaElectricData.map((d) => {
