...
import { reserveProzhektorElectricData } from "../data/reserveProzhektorElectric";
...
export function ReserveProzhektorElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/reserveprozhektor_electric', width, height);
  ...
  const allReserves = reserveProzhektorElectricData.map(d => d.reserve);
  ...
  const allZero = reserveProzhektorElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...reserveProzhektorElectricData.map((d) => {
