...
import { reserveBaskakovoElectricData } from "../data/reserveBaskakovoElectric";
...
export function ReserveBaskakovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/reservebaskakovo_electric', width, height);
  ...
  const allReserves = reserveBaskakovoElectricData.map(d => d.reserve);
  ...
  const allZero = reserveBaskakovoElectricData.every(d => d.reserve === 0);
  ...
  const data1 = [
    ...
    ...reserveBaskakovoElectricData.map((d) => {
