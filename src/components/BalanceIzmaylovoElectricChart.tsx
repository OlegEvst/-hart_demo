...
import { balanceIzmaylovoElectricData } from "../data/balanceIzmaylovoElectric";
...
export function BalanceIzmaylovoElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balanceizmaylovo_electric', width, height);
  ...
  const allValues = [...balanceIzmaylovoElectricData.map(d => d.total_net), ...balanceIzmaylovoElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceIzmaylovoElectricData.map((d) => {
