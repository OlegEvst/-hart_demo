...
import { modelingBalanceProzhektorElectricData } from "../data/modelingBalanceProzhektorElectric";
...
export function ModelingBalanceProzhektorElectricChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingbalanceprozhektor_electric', width, height);
  ...
  const allValues = [...modelingBalanceProzhektorElectricData.map(d => d.total_net), ...modelingBalanceProzhektorElectricData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...modelingBalanceProzhektorElectricData.map((d) => {
