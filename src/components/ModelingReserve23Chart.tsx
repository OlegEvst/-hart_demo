...
import { modelingReserve23Data } from "../data/modelingReserve23";
...
export function ModelingReserve23Chart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/modelingreserve23', width, height);
  ...
  // Кастомные элементы для 2026, 2028 и 2030 годов (только в сжатом положении <= 500px)
  useEffect(() => {
    ...
    const addCustomAnnotations = () => {
      ...
      const totalYears = modelingReserve23Data.length;
      ...
      const value2026 = modelingReserve23Data.find(d => d.year === "2026");
      const value2028 = modelingReserve23Data.find(d => d.year === "2028");
      const value2030 = modelingReserve23Data.find(d => d.year === "2030");
      const yearIndex2026 = value2026 ? modelingReserve23Data.findIndex(d => d.year === "2026") : -1;
      const yearIndex2028 = value2028 ? modelingReserve23Data.findIndex(d => d.year === "2028") : -1;
      const yearIndex2030 = value2030 ? modelingReserve23Data.findIndex(d => d.year === "2030") : -1;
      
      if (!value2026 && !value2028 && !value2030) return false;
      
      // Определяем диапазон vAxis
      const maxReserve = Math.max(...modelingReserve23Data.map(d => d.reserve));
      const vAxisMin = 0;
      ...
      // Адаптивные отступы: при width <= 500px чуть выше точки
      const baseOffset = 15;
      const annotationOffset = baseOffset * 0.9; // Чуть выше точки (опущено ниже)
      
      // Кастомные элементы для 2026 года
      if (value2026 && yearIndex2026 >= 0) {
        ...
        element2026.style.top = `${yPosition2026 - annotationOffset}px`; // Выше точки на 10%
      ...
      if (value2028 && yearIndex2028 >= 0) {
        ...
        element2028.style.top = `${yPosition2028 - annotationOffset}px`; // Выше точки на 10%
      ...
      if (value2030 && yearIndex2030 >= 0) {
        ...
        element2030.style.top = `${yPosition2030 - annotationOffset}px`; // Выше точки на 10%
  ...
  const data1 = [
    ...
    ...modelingReserve23Data.map((d) => [
      d.year,
      d.reserve,
      // В сжатом положении (<= 500px) скрываем стандартные аннотации для 2026, 2028 и 2030 (используем кастомные)
      (isCompressed && (d.year === "2026" || d.year === "2028" || d.year === "2030")) ? "" : Math.round(d.reserve).toString(),
    ]),
  ];
