...
import { balanceKozhukhovoNewData } from "../data/balanceKozhukhovoNew";
...
export function BalanceKozhukhovoNewChart() {
  const [width, height] = useWindowSize();
  const savedConfig = useSavedChartConfig('/balancekozhukhovo_new', width, height);
  ...
  const chartAreaLeft = savedConfig?.chartAreaLeft ?? (isTiny ? "5%" : (isNarrow ? "6%" : (isMobile ? "4%" : "3%")));
  const chartAreaRight = savedConfig?.chartAreaRight ?? (isTiny ? "5%" : (isNarrow ? "6%" : (isMobile ? "4%" : "3%")));
  ...
  const allValues = [...balanceKozhukhovoNewData.map(d => d.total_net), ...balanceKozhukhovoNewData.map(d => d.load)];
  ...
  const data1 = [
    ...
    ...balanceKozhukhovoNewData.map((d) => {
