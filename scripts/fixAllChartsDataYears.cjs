// Скрипт для исправления всех файлов данных - добавляет недостающие годы (2026-2035)
const fs = require('fs');
...
console.log('Исправление данных графиков - добавление недостающих годов...\n');
...
files.forEach(file => {
  ...
  try {
    ...
    console.log(`⚠️  Ошибка парсинга ${file}`);
    return;
  }
  
  checked++;
  
  // Проверяем, какие годы отсутствуют
  ...
  // Определяем тип данных (reserve, value, total_net, load)
